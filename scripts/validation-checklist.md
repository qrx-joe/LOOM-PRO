...
### 3.1 Undo/Redo 历史系统
- [ ] `undo()` 和 `redo()` 内部使用 try-finally，`_skipNextHistory` 在任何路径下都会被重置为 false
- [ ] `saveHistory()` 深克隆失败时不会抛未捕获异常，仅 console.error 并静默跳过
- [ ] `initHistory()` 深克隆失败时降级为空历史（nodes: [], edges: []），不阻断页面加载
- [ ] 结构性变更（addNodes/removeNode/removeEdge/addEdge）自动调用 saveHistory
- [ ] 配置变更（updateNodeData/updateEdgeData）500ms 防抖后调用 saveHistory
- [ ] historyIndex 越界检查：undo 时 <=0 返回 null，redo 时 >=length-1 返回 null

### 3.2 Schema-driven 属性面板
- [ ] 所有已知节点类型（llm/knowledge/condition/code/end/http/trigger）均有对应的 NODE_TYPE_CONFIGS schema
- [ ] `getFieldValue` 对旧工作流 `timeout` → `timeoutMs` 做了向后兼容映射
- [ ] `isFieldVisible` 对 `visibleWhen` 抛异常做了降级处理（默认显示）
- [ ] 未知节点类型时显示警告提示，而非空白面板
- [ ] dataset 字段从 knowledgeStore 动态拉取选项

### 3.3 Text Splitter
- [ ] 构造函数校验 chunkOverlap < chunkSize，否则抛明确错误
- [ ] semanticChunk 的 overlap 计算使用 Math.max(0, ...) 防止负索引
- [ ] 三种策略（fixed/semantic/recursive）的 splitText 入口均正常工作
...
## 五、回归检查（Regression）
- [ ] WorkflowView.vue 不再手动调用 saveHistory，全部通过 store action 委托
- [ ] useWorkflowEdgeHandlers 已删除 dead code（handleEdgeClick）
- [ ] NodeConfigDrawer.vue 和 NodePolicyFields.vue 已被删除且无引用残留
- [ ] PropertiesPanel 的 onError 选项与后端枚举一致（fail/skip/rollback/compensate）
